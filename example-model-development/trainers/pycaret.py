...
class PycaretClassifier:
    ...
    def __init__(
        ...
        Args:
            ...
            n_best_models (int, optional): Number of best models to be logged
                in MLFlow. Defaults to 3.
            data_split_stratify (bool, optional): If False then data is not
                stratified. Defaults to True.
            nfolds (int, optional): Number of folds for cross validation.
                Defaults to 5.
            normalize (bool, optional): If true then features are normalized.
                Defaults to True.
            transformation (bool, optional): If True then features a
                transformed to fit normal distribution. Defaults to True.
            ignore_low_variance (bool, optional): If True low variance is
                ignores. Defaults to True.
            remove_multicollinearity (bool, optional): If True removes
                multicollinearity. Defaults to True.
            multicollinearity_threshold (float, optional): Multicolinearity
    ...
    def mlflow_runs(self):
        try:
            ...
            experiment = PycaretClassifierModule.setup(  # noqa
                experiment_name=self.experiment_name,
                silent=True,
                data=self.df,
                target=self.target,
                data_split_stratify=self.data_split_stratify,
                # Changes the magnitude of features
                # (features with values in similar order)
            ...
            for i in range(len(best_models)):
                ...
                model = best_models[i]
                df_predict = PycaretClassifierModule.predict_model(model, verbose=False)  # noqa
                df_metrics = PycaretClassifierModule.pull()

                # 4) MLFlow Logs
                with mlflow.start_run(
                    run_name=f"pycaret_{self.sort_metric}_{i}"
                ):
                    # Logging Parameters
                    ...
                    try:
                        predict_df = PycaretClassifierModule \
                            .predict_model(
                                model, probability_threshold=self.threshold
                            )
                        predict_df['Class'] = predict_df['Class'].astype(int)
                        predict_df['prob_event'] = model \
                            .predict_proba(
                                predict_df[predict_df.columns[:-3]]
                            ).T[1]
                        ...
                        ks_result = ks_test(
                            df=predict_df,
                            col_target='Class',
                            col_probability='prob_event'
                        )
                        mlflow.log_metric(
                            "ks", round(ks_result['max_ks']/100, 2)
                        )
                        ks_result['ks_table'].to_csv('ks_table.csv')
                        mlflow.log_artifact('ks_table.csv')
                        os.remove('ks_table.csv')
                    except Exception as e:
                        print(f'Error in logging ks for \
                            pycaret_{self.sort_metric}_{i}\n{e}')
                    ...
                    # Logging Artefacts
                    model_plots = [
                        'auc', 'pr', 'confusion_matrix', 'error'                # noqa
                        'class_report', 'boundary', 'manifold', 'calibration',  # noqa
                        'dimension', 'feature', 'parameter'                     # noqa
                    ]

                    for p in model_plots:
                        try:
                            file = PycaretClassifierModule \
                                .plot_model(model, plot=p, save=True)
                            mlflow.log_artifact(file)
                            os.remove(file)
                        except Exception as e:
                            print(f'Error in logging {p} for \
                                pycaret_{self.sort_metric}_{i}\n{e}')
