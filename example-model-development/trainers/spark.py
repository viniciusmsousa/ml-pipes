...
def classification_metrics(
    ...
    Args:
        dfs_prediction (pyspark.sql.dataframe.DataFrame): SparkDataFrame with
        label and prediction columns.
        col_target (str): Label colum name.
        print_metrics (bool, optional): Whether or not to print the metrics.
        Defaults to False.
    ...
    try:
        # Confusion Matrix
        confusion_matrix = dfs_prediction\
            .groupBy(col_target, "prediction").count()
        TN = dfs_prediction\
            .filter(f'prediction = 0 AND {col_target} = 0').count()
        TP = dfs_prediction\
            .filter(f'prediction = 1 AND {col_target} = 1').count()
        FN = dfs_prediction\
            .filter(f'prediction = 0 AND {col_target} = 1').count()
        FP = dfs_prediction\
            .filter(f'prediction = 1 AND {col_target} = 0').count()

        # Computing Metrics from Confusion Matrix
        accuracy = (TN + TP) / (TN + TP + FN + FP)
        precision = TP/(TP+FP) if (TP+FP) > 0 else 0
        recall = TP/(TP+FN)
        f1 = 2*(precision*recall/(precision+recall)) if (precision+recall) > 0 else 0  # noqa
        ...
        df_predictions = dfs_prediction.rdd.map(
            lambda row: (row.prediction, ) +
                tuple(row.probability.toArray().tolist())  # noqa
        ).toDF(["prediction"])
        df_predictions = df_predictions.withColumnRenamed('_3', 'prob_1')
        df_predictions = df_predictions\
            .select('prediction', 'prob_1').toPandas()  # type: ignore
        ks_dict = ks_test(
            df=df_predictions,
            col_target='prediction',
            col_probability='prob_1'
        )
    ...
    def __init__(
        ...
        Args:
            ...
            run_name (str, optional): MLFlow run name.
                Defaults to 'spark_classifier'.
            max_mem_size (str, optional): Max memory to the allocated to Spark.
                Defaults to '4G'.
        ...
        self.spark = init_spark_session(
            n_cores=self.n_cores, max_mem_size=self.max_mem_size
        )
        ...
        self.classfiers = self.classfiers()  # type: ignore

        self.shutdown_spark()

    def get_feature_cols(self):
        try:
           return list(self.df.columns[self.df.columns != self.target_col])  # noqa
        except Exception as e:
            raise Exception(e)
    ...
    def transform_spark(self):
        try:
            ...
            assembler = VectorAssembler(
                inputCols=self.feature_cols, outputCol='features'
            )
    ...
    def classfiers(self):
        try:
            lr = LogisticRegression(
                labelCol=self.target_col, featuresCol='features'
            )
            ...
            for name, model in zip(names, models):
                ...
                with mlflow.start_run(run_name=f'{self.run_name}_{name}'):
